...
public class CommentWriteService {
	...
	public void replyInsert(Long commentNo,String replyContent,
			String cuserId, HttpSession session)
					throws Exception{
		String userId = 
				((AuthInfo)session.getAttribute("authInfo")).getId();
	...
	public String writerInfo(Long commentNo , Model model) 
			throws Exception{
		String path = "";
		/*
		CommentDTO dto =
				commentMapper.getcommentUser(commentNo);
		model.addAttribute("comment", dto);
		return "thymeleaf/comment/writerInfo";
		*/
		CommentUserDTO commentUser = 
				commentMapper.getCommentUserDTO(commentNo);
