...
public class CommentController {
	...
	public String replyInsert(
			...
			@RequestParam(value = "replyContent") 
							String replyContent,
			HttpSession session) throws Exception{
		commentWriteService.replyInsert(commentNo,replyContent,cuserId
				,session);
		return 
		"redirect:/comment/commentDetail?commentNo="+commentNo;
	}
	@RequestMapping(value = "writerInfo" ,method = RequestMethod.POST)
	public String writerInfo(@RequestParam(value = "commentNo")
		Long commentNo, Model model) 
				throws Exception{
		String path = commentWriteService.writerInfo(commentNo,model );
		return path;
	}
}
